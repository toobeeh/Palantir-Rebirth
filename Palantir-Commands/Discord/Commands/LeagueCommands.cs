    ...
    Members.MembersClient membersClient)
{
    ...
    public async Task ViewSeasonOverview(CommandContext context, int? seasonMonth = null, int? seasonYear = null)
    {
        ...
        void AddBriefRank(int indexStart, int indexEnd)
        {
            ...
            var description = string.Join("\n", season.ScoreRanking.ToList()
                .GetRange(indexStart, indexEnd - indexStart + 1)
                .Select((rnk, index) =>
        ...
        // add ranks
        if (season.ScoreRanking.Count == 0)
            embedBuilder.AddField("No participants", "No one has caught drops in this season yet.");
        else AddTopRank(0);
    ...
    public async Task ViewSeasonRank(CommandContext context, int? seasonMonth = null, int? seasonYear = null)
    {
        ...
        {
            rank = await leaguesClient.EvaluateMemberCurrentLeagueSeasonAsync(new EvaluateMemberCurrentSeasonRequest
                { Login = member.Login });
            season = await leaguesClient.EvaluateCurrentLeagueSeasonAsync(new Empty());
        }
        else
    ...
    public async Task ViewSeasonBoard(CommandContext context, int? seasonMonth = null, int? seasonYear = null)
    {
        ...
        var pages = season.ScoreRanking.Chunk(batchSize).Select((batch, idx) => new
        {
            Page = idx + 1,
            Ranks = batch
        }).Select(page =>
        {
            ...
            embedBuilder.WithDescription(
                $"This board shows the ranking of all season participants ordered by drop weight score.\n_ _\n" +
                $"```md\n{rankHeader}\n{string.Join("\n", rankText)}\n```");
            return embedBuilder;
        }).ToList();
